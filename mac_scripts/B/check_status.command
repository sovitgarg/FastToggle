#!/bin/bash
# Check toggle status for all URLs in ToggleExcel.xlsx
...
mkdir -p output
mv status_report.xlsx output/status_report.xlsx 2>/dev/null
mv status_check_*.log ../../_system/logs/ 2>/dev/null

echo ""
echo "Status Check Complete! Results in 'output' folder"
read -p "Press Enter to exit..."
